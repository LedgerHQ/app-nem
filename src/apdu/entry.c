        ...
        TRY {
            ...
            switch (cmd->ins) {
                case INS_GET_PUBLIC_KEY:
                    handle_public_key(cmd->p1, cmd->p2, cmd->data, cmd->lc, flags, tx);
                    break;

                case INS_SIGN:
                    handle_sign(cmd->p1, cmd->p2, cmd->data, cmd->lc, flags);
                    break;

                case INS_GET_REMOTE_ACCOUNT:
                    handle_remote_private_key(cmd->p1, cmd->p2, cmd->data, cmd->lc, flags, tx);
                    break;
