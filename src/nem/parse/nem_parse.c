...
typedef struct importance_txn_header_t {
    // Importance transfer mode. The following modes are supported: 0x01 (Activate), 0x02
    // (Deactivate)
    uint32_t iMode;
...
#define BAIL_IF_ERR(x, err) \
    {                       \
        if (x) return err;  \
    }

int parse_multisig_transaction(parse_context_t *context,
                               common_txn_header_t *common_header,
                               uint8_t is_inner_tx);
...
static int _set_field_data(field_t *field,
                           uint8_t id,
                           uint8_t data_type,
                           uint32_t length,
                           const uint8_t *data) {
...
static int set_field_data(parse_context_t *context,
                          int idx,
                          uint8_t id,
                          uint8_t data_type,
                          uint32_t length,
...
static int add_new_field(parse_context_t *context,
                         uint8_t id,
                         uint8_t data_type,
                         uint32_t length,
                         const uint8_t *data) {
...
static int parse_transfer_transaction(parse_context_t *context,
                                      common_txn_header_t *common_header) {
    char str[32];
    const uint8_t *ptr;
    const uint8_t *startPtr;
    transfer_txn_header_t *txn = (transfer_txn_header_t *) read_data(
        context,
        sizeof(transfer_txn_header_t));  // Read data and security check
    ...
    BAIL_IF(add_new_field(context,
                          NEM_STR_RECIPIENT_ADDRESS,
                          STI_ADDRESS,
                          NEM_ADDRESS_LENGTH,
                          (const uint8_t *) &txn->recipient.address));
    if (common_header->version == 1) {  // NEM transfer tx version 1
        // Show xem amount
        BAIL_IF(add_new_field(context,
                              NEM_MOSAIC_AMOUNT,
                              STI_NEM,
                              sizeof(uint64_t),
                              (const uint8_t *) &txn->amount));
    }
    if (txn->msgLen == 0) {
        // empty msg
        BAIL_IF(add_new_field(context,
        ...
        if (payloadType == 1) {
            // Show Message
            BAIL_IF(
                add_new_field(context,
                              NEM_STR_TXN_MESSAGE,
                              STI_MESSAGE,
                              payloadLength,
                              read_data(context, payloadLength)));  // Read data and security check
        } else {                                                    // show <encrypted msg>
            BAIL_IF(
                add_new_field(context, NEM_STR_ENC_MESSAGE, STI_MESSAGE, 0, (const uint8_t *) ptr));
        }
    }
    // Show fee
    BAIL_IF(add_new_field(context,
                          ...
                          STI_NEM,
                          sizeof(uint64_t),
                          (const uint8_t *) &common_header->fee));
    if (common_header->version == 2) {  // NEM transfer tx version 2
        ...
        if (numMosaic == 0) {
            ...
            for (uint32_t i = 0; i < numMosaic; i++) {
                ...
                BAIL_IF_ERR(mosaicLen - sizeof(uint32_t) - mosaicIdLen - sizeof(uint64_t) != 0,
                            E_INVALID_DATA);
                ...
                BAIL_IF_ERR(
                    mosaicIdLen - sizeof(uint32_t) - nsIdLen - sizeof(uint32_t) - mosaicNameLen !=
                        0,
                    E_INVALID_DATA);
                // mosaic name
                ...
                if (is_nem == 1 && strcmp(str, STR_XEM) == 0) {
                    ...
                    BAIL_IF(add_new_field(
                        ...
                        STI_NEM,
                        sizeof(uint64_t),
                        read_data(context, sizeof(uint64_t))));  // Read data and security check
                } else {
                    if (numMosaic == 1) {
                        BAIL_IF(add_new_field(context,
                                              NEM_UINT32_MOSAIC_COUNT,
                    ...
                    // Unknown mosaic notification
                    BAIL_IF(add_new_field(context,
                                          NEM_MOSAIC_UNKNOWN_TYPE,
                                          STI_STR,
                                          0,
                                          (const uint8_t *) &is_nem));
                    // Show mosaic information: namespace: mosaic name, data=len namespaceId,
                    // namespaceId, len mosaic name, mosaic name
                    BAIL_IF(add_new_field(context,
                                          NEM_STR_TRANSFER_MOSAIC,
                                          STI_STR,
                                          mosaicIdLen,
                                          (const uint8_t *) startPtr));
                    // Mosaic quantity
...
static int parse_importance_transfer_transaction(parse_context_t *context,
                                                 common_txn_header_t *common_header) {
    importance_txn_header_t *txn = (importance_txn_header_t *) read_data(
        context,
        sizeof(importance_txn_header_t));  // Read data and security check
    BAIL_IF_ERR(txn == NULL, E_NOT_ENOUGH_DATA);
    ...
    BAIL_IF(add_new_field(context,
                          ...
                          STI_UINT32,
                          sizeof(uint8_t),
                          (const uint8_t *) &txn->iMode));
    // Show public key of remote account
    BAIL_IF(add_new_field(context,
                          NEM_PUBLICKEY_IT_REMOTE,
                          STI_ADDRESS,
                          NEM_PUBLIC_KEY_LENGTH,
                          (const uint8_t *) &txn->iPublicKey.publicKey));
    // Show fee
    BAIL_IF(add_new_field(context,
                          NEM_UINT64_TXN_FEE,
                          STI_NEM,
...
static int parse_aggregate_modification_transaction(parse_context_t *context,
                                                    common_txn_header_t *common_header) {
    ...
    BAIL_IF(add_new_field(context,
                          NEM_UINT32_AM_COSIGNATORY_NUM,
                          STI_UINT32,
                          sizeof(uint32_t),
                          (const uint8_t *) pcmNum));
    for (uint32_t i = 0; i < cmNum; i++) {
        aggregate_modication_header_t *txn = (aggregate_modication_header_t *) read_data(
            context,
            sizeof(aggregate_modication_header_t));  // Read data and security check
        BAIL_IF_ERR(txn == NULL, E_NOT_ENOUGH_DATA);
        BAIL_IF_ERR(txn->amPublicKey.length > NEM_PUBLIC_KEY_LENGTH, E_INVALID_DATA);
        //  Show modification type
        BAIL_IF(add_new_field(context,
                              NEM_UINT32_AM_MODICATION_TYPE,
                              STI_UINT32,
                              sizeof(uint32_t),
                              (const uint8_t *) &txn->amType));
        // Show public key of cosignatory
        BAIL_IF(add_new_field(context,
                              NEM_PUBLICKEY_AM_COSIGNATORY,
                              STI_ADDRESS,
                              NEM_PUBLIC_KEY_LENGTH,
                              (const uint8_t *) &txn->amPublicKey.publicKey));
    }
    if (common_header->version == 2) {
        ...
        if (cmLen > 0) {
            ...
            BAIL_IF(add_new_field(context,
                                  NEM_UINT32_AM_RELATIVE_CHANGE,
                                  STI_UINT32,
                                  sizeof(uint32_t),
                                  (const uint8_t *) pminCm));  // Read data and security check
        } else {
...
static int parse_multisig_signature_transaction(parse_context_t *context,
                                                common_txn_header_t *common_header) {
    multsig_signature_header_t *txn = (multsig_signature_header_t *) read_data(
        context,
        sizeof(multsig_signature_header_t));  // Read data and security check
    BAIL_IF_ERR(txn == NULL, E_NOT_ENOUGH_DATA);
    ...
    BAIL_IF(add_new_field(context,
                          ...
                          STI_HASH256,
                          txn->hashLen,
                          (const uint8_t *) &txn->hash));
    // Show multisig address
    BAIL_IF(add_new_field(context,
                          NEM_STR_MULTISIG_ADDRESS,
                          STI_ADDRESS,
                          NEM_ADDRESS_LENGTH,
                          (const uint8_t *) &txn->msAddress.address));
    // Show multisig signature inner transaction
    BAIL_IF(parse_multisig_transaction(context, common_header, 0));
    return E_SUCCESS;
}

static int parse_provision_namespace_transaction(parse_context_t *context,
                                                 common_txn_header_t *common_header) {
    rental_header_t *txn =
        (rental_header_t *) read_data(context,
                                      sizeof(rental_header_t));  // Read data and security check
    BAIL_IF_ERR(txn == NULL, E_NOT_ENOUGH_DATA);
    ...
    BAIL_IF(add_new_field(context,
                          ...
                          STI_STR,
                          len,
                          read_data(context, len)));  // Read data and security check
    BAIL_IF(_read_uint32(context, &len));
    if (len == UINT32_MAX) {
        // Show create new root namespace
        BAIL_IF(add_new_field(context,
                              NEM_STR_ROOT_NAMESPACE,
                              STI_STR,
                              sizeof(uint32_t),
                              (const uint8_t *) &len));
    } else {
    ...
    // Show sink address
    BAIL_IF(add_new_field(context,
                          NEM_STR_SINK_ADDRESS,
                          STI_ADDRESS,
                          txn->rAddress.length,
                          (const uint8_t *) &txn->rAddress.address));
    // Show rental fee
    BAIL_IF(add_new_field(context,
                          NEM_UINT64_RENTAL_FEE,
                          STI_NEM,
                          sizeof(uint64_t),
                          (const uint8_t *) &txn->rentalFee));
    // Show fee./
    BAIL_IF(add_new_field(context,
                          NEM_UINT64_TXN_FEE,
                          STI_NEM,
...
static int parse_mosaic_definition_creation_transaction(parse_context_t *context,
                                                        common_txn_header_t *common_header) {
    const uint8_t *ptr;
    uint32_t mdsLen;
    ...
    BAIL_IF(add_new_field(context,
                          ...
                          STI_STR,
                          nsIdLen,
                          read_data(context, nsIdLen)));  // Read data and security check
    // Length of mosaic name string
    uint32_t mosaicNameLen;
    BAIL_IF(_read_uint32(context, &mosaicNameLen));
    BAIL_IF_ERR(midsLen - sizeof(uint32_t) - nsIdLen - sizeof(uint32_t) - mosaicNameLen != 0,
                E_INVALID_DATA);
    ...
    BAIL_IF(add_new_field(context,
                          ...
                          STI_STR,
                          mosaicNameLen,
                          read_data(context, mosaicNameLen)));  // Read data and security check
    ...
    BAIL_IF(add_new_field(context,
                          ...
                          STI_STR,
                          desLen,
                          read_data(context, desLen)));  // Read data and security check
    ...
    for (uint32_t i = 0; i < propertyNum; i++) {
        ...
        BAIL_IF_ERR(
            proStructLen - sizeof(uint32_t) - proNameLen - sizeof(uint32_t) - proValLen != 0,
            E_INVALID_DATA);
    ...
    if (levyLen > 0) {
        ...
        levy_structure_t *levy = (levy_structure_t *) read_data(
            context,
            sizeof(levy_structure_t));  // Read data and security check
        ...
        BAIL_IF_ERR(levy->msIdLen - sizeof(uint32_t) - nsidLen - sizeof(uint32_t) - mnLen != 0,
                    E_INVALID_DATA);
        BAIL_IF_ERR(levyLen - sizeof(levy_structure_t) - levy->msIdLen - sizeof(uint64_t) != 0,
                    E_INVALID_DATA);
        // mosaic name
        BAIL_IF_ERR(move_pos(context, mnLen) == NULL, E_NOT_ENOUGH_DATA);
        // Show levy mosaic: namespace: mosaic name, data = len namespaceid, namespaceid, len mosaic
        // name, mosaic name
        BAIL_IF(add_new_field(context,
                              NEM_STR_LEVY_MOSAIC,
                              STI_STR,
        ...
        // Show levy address
        BAIL_IF(add_new_field(context,
                              NEM_STR_LEVY_ADDRESS,
                              STI_ADDRESS,
                              NEM_ADDRESS_LENGTH,
                              (const uint8_t *) &levy->lsAddress.address));
        // Show levy fee type
        BAIL_IF(add_new_field(context,
                              NEM_UINT32_LEVY_FEE_TYPE,
                              STI_UINT32,
                              sizeof(uint32_t),
                              (const uint8_t *) &levy->feeType));
        // Show levy fee
        BAIL_IF(
            add_new_field(context,
                          NEM_UINT64_LEVY_FEE,
    ...
    BAIL_IF_ERR(mdsLen - sizeof(publickey_t) - sizeof(uint32_t) - midsLen - sizeof(uint32_t) -
                        desLen - sizeof(uint32_t) - propertyLen - sizeof(uint32_t) - levyLen !=
                    0,
                E_INVALID_DATA);
    // Check mosaic definition sink address
    mosaic_definition_sink_t *sink = (mosaic_definition_sink_t *) read_data(
        context,
        sizeof(mosaic_definition_sink_t));  // Read data and security check
    ...
    BAIL_IF(add_new_field(context,
                          ...
                          STI_ADDRESS,
                          NEM_ADDRESS_LENGTH,
                          (const uint8_t *) &sink->mdAddress.address));
    // Show rentail fee
    BAIL_IF(add_new_field(context,
                          NEM_UINT64_RENTAL_FEE,
                          STI_NEM,
                          sizeof(uint64_t),
                          (const uint8_t *) &sink->fee));
    // Show tx fee
    BAIL_IF(add_new_field(context,
                          NEM_UINT64_TXN_FEE,
                          STI_NEM,
...
static int parse_mosaic_supply_change_transaction(parse_context_t *context,
                                                  common_txn_header_t *common_header) {
    // Length of mosaic id structure
    uint32_t msidLen;
    ...
    BAIL_IF_ERR(msidLen - sizeof(uint32_t) - nsidLen - sizeof(uint32_t) - msnLen != 0,
                E_INVALID_DATA);
    // Show mosaic name string
    BAIL_IF(add_new_field(context,
                          NEM_STR_MOSAIC,
                          STI_STR,
                          msnLen,
                          read_data(context, msnLen)));  // Read data and security check
    ...
    if (supplyType == 1) {
        BAIL_IF(
            add_new_field(context,
                          NEM_MOSAIC_CREATE_SUPPLY_DELTA,
                          STI_MOSAIC_CURRENCY,
                          sizeof(uint64_t),
                          read_data(context, sizeof(uint64_t))));  // Read data and security check
        ...
        BAIL_IF(
            add_new_field(context,
                          NEM_MOSAIC_DELETE_SUPPLY_DELTA,
                          STI_MOSAIC_CURRENCY,
                          sizeof(uint64_t),
                          read_data(context, sizeof(uint64_t))));  // Read data and security check
    } else {
...
int parse_multisig_transaction(parse_context_t *context,
    ...
    while (innerOffset < innerTxnLength) {
        ...
        // get header first
        common_txn_header_t *inner_header = (common_txn_header_t *) read_data(
            context,
            sizeof(common_txn_header_t));  // Read data and security check
        BAIL_IF_ERR(inner_header == NULL, E_NOT_ENOUGH_DATA);
        // Show inner transaction / detail transaction type
        BAIL_IF(add_new_field(
            context,
            is_inner_tx ? NEM_UINT32_INNER_TRANSACTION_TYPE : NEM_UINT32_DETAIL_TRANSACTION_TYPE,
            STI_UINT32,
            sizeof(uint32_t),
            (const uint8_t *) &inner_header->transactionType));
...
static int parse_txn_detail(parse_context_t *context, common_txn_header_t *common_header) {
    ...
    BAIL_IF(add_new_field(context,
                          NEM_UINT32_TRANSACTION_TYPE,
                          STI_UINT32,
                          sizeof(uint32_t),
                          (const uint8_t *) &common_header->transactionType));
    switch (common_header->transactionType) {
        case NEM_TXN_TRANSFER:
            ret = parse_transfer_transaction(context, common_header);
            break;
        case NEM_TXN_IMPORTANCE_TRANSFER:
            ret = parse_importance_transfer_transaction(context, common_header);
            break;
        case NEM_TXN_MULTISIG_AGGREGATE_MODIFICATION:
            ret = parse_aggregate_modification_transaction(context, common_header);
            break;
        case NEM_TXN_MULTISIG_SIGNATURE:
            ret = parse_multisig_signature_transaction(context, common_header);
            break;
        case NEM_TXN_MULTISIG:
            ret = parse_multisig_transaction(context, common_header, 1);
            break;
        case NEM_TXN_PROVISION_NAMESPACE:
            ret = parse_provision_namespace_transaction(context, common_header);
            break;
        case NEM_TXN_MOSAIC_DEFINITION:
            ret = parse_mosaic_definition_creation_transaction(context, common_header);
            break;
        case NEM_TXN_MOSAIC_SUPPLY_CHANGE:
            ret = parse_mosaic_supply_change_transaction(context, common_header);
            break;
        default:
            ret = E_INVALID_DATA;
            break;
...
static void set_sign_data_length(parse_context_t *context) {
    if (context->transactionType == NEM_TXN_MULTISIG_SIGNATURE) {
        ...
        transactionContext.rawTxLength =
            sizeof(multsig_signature_header_t) + sizeof(common_txn_header_t);
    } else {
...
static common_txn_header_t *parse_common_header(parse_context_t *context) {
    ...
    common_txn_header_t *common_header = (common_txn_header_t *) read_data(
        context,
        sizeof(common_txn_header_t));  // Read data and security check
